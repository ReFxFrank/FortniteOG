...
static TLazyDefaultObject<UKismetMathLibrary> Math;
static TLazyDefaultObject<UGameplayStatics> Statics;
...
public:
	...
	FactionBot* CurrentAssignedDBNOBot = nullptr;
...
public:
	void Tick(FactionBot* bot) {
		...
		{
			AFortPlayerPawnAthena* TargetPawn = Cast<AFortPlayerPawnAthena>(bot->CurrentTarget);
			if (TargetPawn && (TargetPawn->GetHealth() <= 0.f || TargetPawn->bIsDBNO || TargetPawn->bIsDying))
			{
				bot->CurrentTarget = nullptr;
				bot->PC->CurrentAlertLevel = EAlertLevel::LKP;
			}
		}
		...
		if (bot->bIsReviving && (bot->bIsStressed || bot->CurrentTarget || bot->PC->CurrentAlertLevel == EAlertLevel::Threatened || bot->PC->CurrentAlertLevel == EAlertLevel::Alerted || bot->PC->CurrentAlertLevel == EAlertLevel::LKP)) {
			bot->bIsReviving = false;
			bot->PC->ReviveTarget = nullptr;
		}

		if (bot->bIsReviving && bot->CurrentAssignedDBNOBot && bot->CurrentAssignedDBNOBot->Pawn) {
			...
			if (Statics->GetTimeSeconds(UWorld::GetWorld()) >= bot->ReviveTime) {
				...
				if (bot->Weapon && (bot->WeaponGuid.A != 0 || bot->WeaponGuid.B != 0 || bot->WeaponGuid.C != 0 || bot->WeaponGuid.D != 0))
...
namespace Bosses {
	static std::map<FactionBot*, int> LostCombatLOSTicks;
	...
	inline void CleanupBotData(FactionBot* bot)
	{
		LostCombatLOSTicks.erase(bot);
	}

	inline FRotator LookAtWithPitch(FactionBot* bot, FVector TargetPos)
	{
		if (!bot || !bot->Pawn)
			return FRotator();
		...
		FVector BotPos = bot->Pawn->K2_GetActorLocation();
		TargetPos.Z += 88.f;
		return Math->FindLookAtRotation(BotPos, TargetPos);
	}

	inline void MoveTowardAssignedDBNO(FactionBot* bot, FVector BotPos, float Distance)
	{
		if (!bot || !bot->Pawn || !bot->PC || !bot->CurrentAssignedDBNOBot || !bot->CurrentAssignedDBNOBot->Pawn)
			return;

		FVector DBNOLoc = bot->CurrentAssignedDBNOBot->Pawn->K2_GetActorLocation();
		FVector ToDBNO = DBNOLoc - BotPos;
		ToDBNO.Normalize();

		bool bHasLOS = bot->PC->LineOfSightTo(bot->CurrentAssignedDBNOBot->Pawn, BotPos, true);
		if (!bHasLOS && Distance < 600.f)
		{
			if (bot->Pickaxe && IsValidGuid(bot->PickaxeGuid))
				bot->Pawn->EquipWeaponDefinition((UFortWeaponItemDefinition*)bot->Pickaxe, bot->PickaxeGuid);

			FRotator FaceRot = Math->FindLookAtRotation(BotPos, DBNOLoc);
			FaceRot.Pitch = 0.f;
			bot->PC->SetControlRotation(FaceRot);
			bot->PC->K2_SetActorRotation(FaceRot, true);
			bot->Pawn->AddMovementInput(ToDBNO, 0.65f, true);
			bot->Pawn->PawnStartFire(0);
		}
		else
		{
			if (bot->Weapon && IsValidGuid(bot->WeaponGuid))
				bot->Pawn->EquipWeaponDefinition(bot->Weapon, bot->WeaponGuid);
			bot->Pawn->PawnStopFire(0);
			bot->PC->MoveToActor(bot->CurrentAssignedDBNOBot->Pawn, 100, true, false, true, nullptr, true);
		}
	}
	inline void TickBots(float DeltaTime)
	{
		auto block = [](FactionBot* bot, std::function<void(FactionBot* bot)> const& SetUnaware, bool Alerted, bool Threatened, bool LKP) {
			...
			if (!Threatened && !Alerted && !LKP && !bot->CurrentTarget && !bot->bIsStressed && bot->CurrentAssignedDBNOBot && bot->CurrentAssignedDBNOBot->Pawn && bot->CurrentAssignedDBNOBot->Pawn->bIsDBNO && bot->PC->CurrentAlertLevel == EAlertLevel::Unaware) {
				FVector BotPos = bot->Pawn->K2_GetActorLocation();
				float Distance = Math->Vector_Distance(BotPos, bot->CurrentAssignedDBNOBot->Pawn->K2_GetActorLocation());
				auto TestRot = LookAtWithPitch(bot, bot->CurrentAssignedDBNOBot->Pawn->K2_GetActorLocation());

				if (Distance > 800.f) {
					bot->bIsReviving = false;
					bot->PC->ReviveTarget = nullptr;
					bot->CurrentAssignedDBNOBot = nullptr;
					bot->tick_counter++;
					return;
				}
				...
				if (Distance < 200.0f) {
					...
					MoveTowardAssignedDBNO(bot, BotPos, Distance);
				}
			...
			if ((bot->tick_counter % 10 == 0) && bot->bWasReportedStuck && bot->Weapon && IsValidGuid(bot->WeaponGuid)) {
					bot->Pawn->EquipWeaponDefinition(bot->Weapon, bot->WeaponGuid);
			...
			if (Alerted) {
				if (bot->CurrentTarget) {
					...
					auto Rot = LookAtWithPitch(bot, TargetPos);
					...
					if (Math->RandomBoolWithWeight(0.25f)) {
						...
						bot->PC->K2_SetActorRotation(Rot, true);
					                    ...
					                    FVector AdjustedTarget = TargetPos;
                    AdjustedTarget.Z = BotPos.Z;
                    float HeightDiff = TargetPos.Z - BotPos.Z;
                    if (HeightDiff > 250.f && Math->RandomBoolWithWeight(0.06f))
                    {
                        bot->Pawn->UnCrouch(false);
                        bot->Pawn->Jump();
                    }
                    bot->PC->MoveToLocation(AdjustedTarget, 25.f, true, false, false, true, nullptr, true);
					...
					if (Math->RandomBoolWithWeight(0.25f)) {
						...
						Rot = LookAtWithPitch(bot, TargetPosMod);
				...
				if (bot->CurrentTarget) {
					...
					if (!bot->bIsStressed) {
						...
						float HeightDiff = TargetPos.Z - BotPos.Z;
						if (HeightDiff > 200.f && Math->RandomBoolWithWeight(0.04f))
						{
							bot->Pawn->UnCrouch(false);
							bot->Pawn->Jump();
						}
						...
						bot->Pawn->AddMovementInput((bot->Pawn->GetActorForwardVector() * -1.0f), 0.6f, true);
					}
					...
					if (true) { // wouldve had a tickcounter condition here but no
						...
						FRotator Rot = LookAtWithPitch(bot, TargetPosMod);
					...
					else
					{
						if (Statics->GetTimeSeconds(UWorld::GetWorld()) < bot->StrafeEndTime)
						{
							if (bot->StrafeType == EBossesStrafeType::StrafeLeft) {
								bot->Pawn->AddMovementInput((bot->Pawn->GetActorRightVector() * -1.0f), 0.6f, true);
							}
							else {
								bot->Pawn->AddMovementInput(bot->Pawn->GetActorRightVector(), 0.6f, true);
							}
						}
						else
						{
					...
					bool bHasLOS = bot->PC->LineOfSightTo(bot->CurrentTarget, BotPos, true);
					bool HasMinigun = bot->Pawn->CurrentWeapon &&
						bot->Pawn->CurrentWeapon->Name.ToString().contains("Minigun");

					if (bHasLOS) {
						if (bot && bot->Pawn)
							LostCombatLOSTicks[bot] = 0;
						if (HasMinigun) {
							bot->Pawn->PawnStartFire(0);
						}
						else if (bot->tick_counter % 2 == 0) {
							bot->Pawn->PawnStartFire(0);
						}
						else {
							bot->Pawn->PawnStopFire(0);
					...
					else if (bot && bot->Pawn) {
						LostCombatLOSTicks[bot]++;
						if (!HasMinigun || LostCombatLOSTicks[bot] > 3) {
							bot->Pawn->PawnStopFire(0);
							LostCombatLOSTicks[bot] = 0;
						}
					}
