...
namespace Looting {
    ...
    FVector GetContainerLootSpawnLocation(ABuildingContainer* BuildingContainer, bool bFloorLoot)
    {
        FVector BaseLocation = BuildingContainer->K2_GetActorLocation();
        FVector CorrectLocation = BaseLocation
            + (BuildingContainer->GetActorForwardVector() * BuildingContainer->LootSpawnLocation_Athena.X)
            + (BuildingContainer->GetActorRightVector() * BuildingContainer->LootSpawnLocation_Athena.Y)
            + (BuildingContainer->GetActorUpVector() * BuildingContainer->LootSpawnLocation_Athena.Z);

        if (!bFloorLoot)
        {
            const float MaxContainerLift = BaseLocation.Z + 45.f;
            const float MinContainerLift = BaseLocation.Z - 15.f;
            if (CorrectLocation.Z > MaxContainerLift || CorrectLocation.Z < MinContainerLift)
            {
                CorrectLocation.Z = BaseLocation.Z + 22.f;
            }
        }
    ...
    char SpawnLoot(ABuildingContainer* BuildingContainer)
    {
        ...
        else if (Globals::LateGame && SearchLootTierGroup == Loot_AthenaFloorLoot_Warmup || Globals::Arsenal && SearchLootTierGroup == Loot_AthenaFloorLoot_Warmup)
        {
            ...
            auto CorrectLocation = GetContainerLootSpawnLocation(BuildingContainer, PickupSourceTypeFlags == EFortPickupSourceTypeFlag::FloorLoot);
            ...
            for (auto& LootDrop : LootDrops)
            {
                if (LootDrop.Count > 0)
                {
                    FVector SpreadLoc = CorrectLocation;
                    SpreadLoc.X += (rand() % 80) - 40;
                    SpreadLoc.Y += (rand() % 80) - 40;
                    SpawnPickup(LootDrop.ItemDefinition, LootDrop.Count, LootDrop.LoadedAmmo, SpreadLoc, PickupSourceTypeFlags, SpawnSource);
                    ...
                    if (SearchLootTierGroup == Loot_AthenaFloorLoot || SearchLootTierGroup == Loot_AthenaFloorLoot_Warmup)
                    {
                        ...
                        {
                            FVector AmmoSpreadLoc = SpreadLoc;
                            AmmoSpreadLoc.X += (rand() % 40) - 20;
                            AmmoSpreadLoc.Y += (rand() % 40) - 20;
                            SpawnPickup(AmmoDef, AmmoDef->DropCount, 0, AmmoSpreadLoc, PickupSourceTypeFlags, SpawnSource);
                        }
                    }
                }
                ItemIndex++;
        ...
        if (!Globals::LateGame || !Globals::Arsenal)
        {
            ...
            if (ClassName.contains("Tiered_Chest_Athena_FactionChest"))
            {
                for (int i = 0; i < 2; i++)
                {
                    ...
                    {
                        FVector SpreadLoc = CorrectLocation;
                        SpreadLoc.X += (rand() % 100) - 50 + (ItemIndex * 30);
                        SpreadLoc.Y += (rand() % 100) - 50;
                        SpawnPickup(LootDrop.ItemDefinition, LootDrop.Count, LootDrop.LoadedAmmo, SpreadLoc, PickupSourceTypeFlags, SpawnSource);
            ...
            else if (ClassName.contains("Tiered_Chest"))
            {
                ...
                FVector MatsSpreadLoc = CorrectLocation;
                MatsSpreadLoc.X += (rand() % 60) - 30;
                MatsSpreadLoc.Y += (rand() % 60) - 30;
                SpawnPickup(Mats, 30, 0, MatsSpreadLoc, PickupSourceTypeFlags, SpawnSource);
