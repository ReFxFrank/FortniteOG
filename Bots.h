...
namespace Bots {
	...
	inline bool ShouldGiveBotStartupItem(UFortItemDefinition* Item)
	{
		if (!Item)
			return false;
	...
	{
		if (!Globals::bBotsEnabled || !BotManager || !BotData || !BotMutator)
		...
		if (!BotData->CharacterCustomization || !EnsureValidAthenaLoadout(BotData->CharacterCustomization->CustomizationLoadout, "bot data"))
			return SpawnBotOG(BotManager, SpawnLoc, SpawnRot, BotData, RuntimeBotData);
		...
		TryApplyPawnCosmeticLoadout(Ret, "bot pawn");
		...
		BotManagerSetup(__int64(BotManager), __int64(Ret), __int64(BotData->BehaviorTree), 0, &CustomSquadId, 0, 0, __int64(BotData->BotNameSettings), 0, &FalseByte, 0, &TrueByte, RuntimeBotData);

		FactionBot* bot = new FactionBot(Ret);

		bot->CID = BotData->CharacterCustomization->CustomizationLoadout.Character->GetName();

		if (BotData->StartupInventory)
		{
			for (int32 i = 0; i < BotData->StartupInventory->Items.Num(); i++)
			{
				UFortItemDefinition* StartupItem = BotData->StartupInventory->Items[i];
				if (!ShouldGiveBotStartupItem(StartupItem)) {
					continue;
				}

				bool equip = true;

				if (StartupItem->GetName().starts_with("WID_Athena_FloppingRabbit") || StartupItem->GetName().starts_with("WID_Boss_Adventure_GH")) {
					equip = false;
				}
				bot->GiveItem(StartupItem, 1, equip);
			}
		}
		...
		for (int i = 0; i < PatrolPaths.Num(); i++) {
			if (!PatrolPaths[i] || PatrolPaths[i]->PatrolPoints.Num() <= 0 || !PatrolPaths[i]->PatrolPoints[0]) {
				continue;
			}

			if (Math->Vector_Distance(PatrolPaths[i]->PatrolPoints[0]->K2_GetActorLocation(), SpawnLoc) < 350.f) {
	...
	void OnActorBump(UFortAthenaAIBotPathFollowingComponent* Comp, AActor* SelfActor, AActor* OtherActor, const FVector& NormalImpulse, FHitResult& Hit)
	{
		...
		for (auto& bot : FactionBots)
		{
			if (bot->PC == Comp->BotController)
			{
				...
				if (bot->Pickaxe && Bosses::IsValidGuid(bot->PickaxeGuid))
